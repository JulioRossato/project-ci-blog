$(document).on('submit', "[data-module='AttributeValueController']", function (e) {
    e.preventDefault();
        ...
        Swal.fire({
            ...
            preConfirm: function () {
                return new Promise((resolve, reject) => {
                    $.ajax({
                        type: 'POST',
                        url: '/admin/attribute-value/delete',
                        data: dataJson,
                        dataType: 'json',
...
$(document).on('change', '.swatcheType', function (e) {
    e.preventDefault();
    var swatche_type = $(this).val();
    if (swatche_type == "1") {
        $('#swatcheImage').hide();
        $('#swatcheColor').show();
        $('input[name="swatcheImage"]').val('');
    } else if (swatche_type == "2") {
        $('#swatcheColor').hide();
        $('#swatcheImage').show();
        $('input[name="swatcheColor"]').val('');
    } else {
        $('#swatcheColor').hide();
        $('#swatcheImage').hide();
        $('input[name="swatcheColor"]').val('');
